...
def index():
    if request.method == "POST":
        ...
        if (
            data_for_creation.get("google_id")
            and data_for_creation.get("token")
            and data_for_creation.get("admin_key")
        ):
            ...
            hook = requests.get(
                main_url + f"setWebhook?url=alexovidovich.pythonanywhere.com/{bot_n}"
            )
        ...
        if request.method == "POST":
            ...
            try:
                if data:
                    ...
                    data["admins"] = (
                        data_to_read.reading_json("admin")
                        .get(data["bot"])
                        .get("chat_id")
                    )
                    data["delta"] = (
                        data_to_read.reading_json("admin").get(data["bot"]).get("delta")
                    )
                    data["min_order"] = (
                        data_to_read.reading_json("admin")
                        .get(data["bot"])
                    ...
                    if Admin.admin_interceptor(data=data):
                        ...
                        Interceptor.interceptor(
                            data=data,
                            pattern=patterns.get(data["bot"]).get("13"),
                            worker=Handler.if_start,
                        )
                        == True
                    ):
                        print("старт")  # 1
                        ...
                        Interceptor.interceptor(
                            data=data,
                            pattern=patterns.get(data["bot"]).get("14"),
                            worker=Handler.if_rules,
                        )
                        == True
                    ):
                        print("правила")  # 2
                        ...
                        Interceptor.interceptor(
                            data=data,
                            pattern=patterns.get(data["bot"]).get("15"),
                            worker=Handler.if_contacts,
                        )
                        == True
                    ):
                        print("контакты")  # 3
                        ...
                        Interceptor.interceptor(
                            data=data,
                            pattern=patterns.get(data["bot"]).get("16"),
                            worker=Handler.if_calendar,
                        )
                        == True
                    ):
                        print("календарь")  # 4
                    elif (
                        Interceptor.interceptor_for_callback(
                            data=data,
                            worker=[
                        ...
                        Interceptor.interceptor(
                            data=data, pattern=admin_key, worker=Handler.addAdmin
                        )
                        == True
                    ):
                        print("admin check")
                        ...
                        Interceptor.interceptor(
                            data=data,
                            pattern=["админка"],
                            worker=Handler.show_admin_widget,
                        )
                        == True
                    ):
                        print("admin check")
                ...
                Handler.message(
                    data,
                    "Возникли неполадки, не связанные с алгоритмом исполнения. Обратитесь в поддержку.",
                )
